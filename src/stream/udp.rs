...
impl IpStackUdpPacketEndpoint {
    pub fn new(
        ...
        Self { receiver, up_pkt_sender, local_addr, mtu, _destroy_messenger }
    }
    ...
    pub fn send(&self, src: SocketAddr, dst: SocketAddr, payload: Vec<u8>) -> std::io::Result<()> {
        ...
        self.up_pkt_sender.send(raw_packet).map_err(|_| {
            std::io::Error::new(std::io::ErrorKind::BrokenPipe, "ipstack up_pkt_sender closed")
        })
    }
...
pub fn build_raw_udp_packet(
    ...
    match (src_addr.ip(), dst_addr.ip()) {
        (std::net::IpAddr::V4(src), std::net::IpAddr::V4(dst)) => {
            ...
            let mut ip_h = Ipv4Header::new(0, ttl, IpNumber::UDP, src.octets(), dst.octets())
                .map_err(IpStackError::from)?;
            ...
            let udp_header = UdpHeader::with_ipv4_checksum(src_addr.port(), dst_addr.port(), &ip_h, &payload)
                .map_err(IpStackError::from)?;
        ...
        (std::net::IpAddr::V6(src), std::net::IpAddr::V6(dst)) => {
            ...
            payload.truncate(line_buffer as usize);
            ip_h.payload_length = (payload.len() + UHS) as u16;
            let udp_header = UdpHeader::with_ipv6_checksum(src_addr.port(), dst_addr.port(), &ip_h, &payload)
                .map_err(IpStackError::from)?;
